...
const MapsBody = () => {
  ...
  const { needsEventsRefresh, allParkTrails, JWTTokenResult } = useSelector(
  ...
  const getAndSetMyEvents = () => {
    if (currentSkateProfile !== undefined) {
      if (
        ...
        Fetch.getEventsForSkateProfile(
          JWTTokenResult.token,
          currentSkateProfile.id,
          myEvents => {
            setAttendingEvents(myEvents);
          },
          () => {
            setAttendingEvents([]);
            uiUtils.showPopUp(
    ...
    return (
      ...
      >
        <Button
          ...
          text={
            parkTrailsVisible === true ? "Hide ParkTrails" : "Show ParkTrails"
          }
          onPress={() => setParkTrailsVisible(prevValue => !prevValue)}
        ></Button>
      </View>
    );
  };
  ...
  const renderTrail = (trailId: string) => {
    ...
    if (
      ...
      foundEvent.outing !== undefined &&
      foundEvent.outing.trail !== undefined
    ) {
  ...
  return (
    <View>
      ...
      >
        {parkTrailsVisible === true && getAllParkTrailMarkers()}
        {mapsUtils.getAttendingEvents(
          attendingEvents,
          mapsElementCount + 1,
          id => renderTrail(id)
        )}
        {mapsUtils.getRecommendedEvents(
          recommendedEvents,
          mapsElementCount + 1,
          id => renderTrail(id)
        )}
        {trailIsRendered === true &&
          trail !== undefined &&
          mapsUtils.drawRoute(trail.checkPoints, 300)}
        {myLocation !== undefined &&
          mapsUtils.getMarker(
            ...
            "Your location",
            mapsElementCount + recommendedEvents.length + 1
          )}
      </MapView>
      {getTopButtons()}
    </View>
