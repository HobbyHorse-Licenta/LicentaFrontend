...
import { SelectionListModal, Tile } from "../general";
...
const SkateProfileSummaryWithSkills = ({ skateProfileId }: Input) => {
  ...
  const profileInfo = user?.skateProfiles.find(
    ...
    if (profileInfo !== undefined) {
      ...
      if (
        ...
        Fetch.getSkillRecommendations(
          ...
          profileInfo.skatePracticeStyle,
          profileInfo.skateExperience,
          skills =>
  ...
  const removeAssignedSkillsFromRecommendations = (
    ...
    return recomSkills?.filter(recommendedSkill => {
      if (
        ...
        profileInfo.assignedSkills !== undefined &&
        profileInfo.assignedSkills !== null
      ) {
        if (
          profileInfo.assignedSkills.find(
            assignedSkill =>
  ...
  const getSystemGuide = () => {
    ...
    return (
      ...
      >
        ...
        <View>
          {masteringLevelValues.map(value => {
            return (
              <View
              ...
              >
                <View
                  style={{
                    ...
                    height: 10,
                    borderRadius: 50,
                    backgroundColor: uiUtils.getColorBasedOnSkillLevel(value),
                  }}
                ></View>
      ...
      if (user !== undefined) {
        if (
          ...
          Fetch.postAssignedSkill(
            JWTTokenResult.token,
            newAssignedSkill,
            () => console.log("added skill successfully"),
            () => {
              uiUtils.showPopUp(
                "Error",
      ...
      return recommendedSkills.map((recommendedSkill, index) => {
        return {
          ...
          element: (
            ...
            >
              <Text>{recommendedSkill.skill.name}</Text>
            </Tile>
          ),
        };
      });
          ...
          >
            ...
            <View style={styles.skillsContainer}>
              ...
              >
                <AssignedSkillList
                  skateProfileId={profileInfo.id}
                  onPressAddSkill={() => addNewSkillToProfile()}
                ></AssignedSkillList>
              </ScrollView>
            </View>
          </View>
          <View style={styles.systemGuideContainer}>{getSystemGuide()}</View>
            ...
            <View>
              ...
              recommendedSkills.length > 0 ? (
                <SelectionListModal
                  ...
                  list={getRecommendedSkillsElements()}
                ></SelectionListModal>
              ) : (
                <QuestionModal
                  question="No recommended event suggestions"
                  button1Text="Ok"
                  onButton1Press={() => setAddSkillModalVisible(false)}
                  onDismiss={() => nothing}
                  visible={addSkillModalVisible}
                />
              )}
