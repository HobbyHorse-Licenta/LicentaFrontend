...
const SkateProfileSummary = ({info, selected, onLongPress, onPress}: Input) => {
    ...
    const getStyle = () => {
        if(selected === true)
        {
            return {...SpacingStyles.skateProfileSummary,...{backgroundColor: heldDown === false ? theme.colors.tertiary : uiUtils.darkenColor(theme.colors.tertiary, 10), borderColor: theme.colors.tertiary}}
        }
        else return {...SpacingStyles.skateProfileSummary,...{backgroundColor: heldDown === false ? theme.colors.background : uiUtils.darkenColor(theme.colors.background, 10), borderColor: theme.colors.tertiary}}
    }
    const theme = useTheme();
    return(
        <Pressable onTouchStart={() => setHeldDown(true)} onTouchEnd={() => setHeldDown(false)}
