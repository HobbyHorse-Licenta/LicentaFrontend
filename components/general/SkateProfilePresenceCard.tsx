...
const SkateProfilePresenceCard = ({
  skateProfile,
  inactive,
  allUsers,
  cardOpacity,
  ...
  const getViewStyle = () => {
    ...
    if (inactive === true) {
      return { ...commonStyle, backgroundColor: "gray", opacity: 0.5 };
    } else {
      return commonStyle;
    }
  };
  ...
  const assignedSkillsElements = skateProfile.assignedSkills?.map(
    assignedSkill => (
      <AssignedSkillElement
  ...
  return (
    <PrimaryContainer
      styleInput={{
        ...
        height: 150,
        marginHorizontal: 5,
        opacity: cardOpacity !== undefined ? cardOpacity : 1,
      }}
    >
      {user !== undefined && user !== null ? (
        <View style={getViewStyle()}>
          ...
          <Text style={[{ flexWrap: "nowrap" }]} variant="labelSmall">
            {user.name}
          </Text>
          <Text style={{ flexWrap: "nowrap" }} variant="labelSmall">
            {skateProfile.skateExperience}
          </Text>
          {skateProfile !== undefined &&
            skateProfile.assignedSkills !== undefined &&
            skateProfile.assignedSkills.length > 0 &&
