...
const EventCard = ({ event, onPress, joined }: EventInput) => {
  ...
  const [recommendedUsersImagesUrl, setRecommendedUsersImagesUrl] = useState<
    Array<string | undefined> | undefined
  >();
  const [recommendedSkateProfiles, setRecommendedSkateProfiles] =
    useState<Array<SkateProfile>>();
  const [participatingUsersImagesUrl, setParticipatingUsersImagesUrl] =
    useState<Array<string | undefined> | undefined>();
  const [participatingSkateProfiles, setParticipatingSkateProfiles] =
    useState<Array<SkateProfile>>();
    ...
    if (currentSkateProfile !== undefined) {
      if (
        ...
        Fetch.getSuggestedSkateProfilesForEvent(
          ...
          event.id,
          skateProfiles =>
            setRecommendedSkateProfiles(
              filterUtils.excludeSkateProfile(
                skateProfiles,
                currentSkateProfile
              )
            ),
  ...
  useEffect(() => {
    if (
      recommendedSkateProfiles !== undefined &&
      recommendedSkateProfiles.length > 0
    ) {
      setRecommendedUsersImagesUrl(
        getAllPicturesFromSkateProfiles(recommendedSkateProfiles)
      );
    }
  ...
  useEffect(() => {
    if (
      participatingSkateProfiles !== undefined &&
      participatingSkateProfiles.length > 0
    ) {
      setParticipatingUsersImagesUrl(
        getAllPicturesFromSkateProfiles(participatingSkateProfiles)
      );
    }
  }, [participatingSkateProfiles]);

  const getAllPicturesFromSkateProfiles = (
    skateProfiles: Array<SkateProfile>
  ): Array<string | undefined> => {
    const userArray: Array<User> = [];
    skateProfiles.forEach(
      skateProfile =>
        skateProfile.user !== undefined && userArray.push(skateProfile.user)
    );
  ...
  return (
    <Pressable
      onPress={() =>
        onPress != undefined
          ? onPress()
          : console.log("[EventCard]: no action on press")
      }
      style={[
        styles.container,
        { alignSelf: "center" },
        SpacingStyles.eventCard,
      ]}
    >
      {reversed === false ? (
        <View style={{ width: "100%", height: "100%", flexDirection: "row" }}>
          ...
          >
            ...
            <View style={{ width: "80%", margin: "5%", alignSelf: "center" }}>
              <ProfilePicList
                imageUrlsArray={participatingUsersImagesUrl}
                grayedOutImageUrlsArray={recommendedUsersImagesUrl}
              ></ProfilePicList>
            </View>
          </View>
        </View>
