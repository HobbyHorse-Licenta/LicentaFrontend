...
const SelectLocationCasualAndSpeed = ({
  onTouchInside,
  onTouchOutside,
  setParkSelected,
  zone,
  ...
  const [localZone, setLocalZone] = useState<Zone | undefined>(
  ...
  //////FOR WALKTHROUGH////
  const { TourGuideZone } = useTourGuideController("schedule");
  ...
  const findRangeBiggerThanDistance = (distance: number): number => {
    const range = rangeArray.find(({ value }) => value >= distance);
    if (range !== undefined) {
      return range.value;
    } else {
      return 100;
  ...
  const increaseRangeSoThatParkTrailIsInRange = (parkTrail: ParkTrail) => {
    if (
      localZone !== undefined &&
      localZone !== null &&
      localZone.location !== undefined &&
      localZone.location !== null
    ) {
      const distance = mapsUtils.getDistanceInKmBetweenLocations(
        localZone.location,
        parkTrail.location
      );
  ...
  const getParkLocations = () => {
    if (allParkTrails !== undefined && allParkTrails !== null) {
      return (
        <View>
          {allParkTrails.map(parkTrail => {
            let parkOptionStyle: ViewStyle = {
              borderWidth: 1,
            ...
            return (
              <PrimaryContainer
                ...
                onPress={() => increaseRangeSoThatParkTrailIsInRange(parkTrail)}
                styleInput={{ ...parkOptionStyle, margin: scale(3) }}
              >
