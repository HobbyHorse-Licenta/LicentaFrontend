...
const MySchedulesBody = () => {
  const { user, JWTTokenResult, needsSchedulesRefresh } = useSelector(
    (state: RootState) => state.appState
  );
  const { currentSkateProfile } = useSelector(
    (state: RootState) => state.globalState
  );
  const [schedulesToDisplay, setSchedulesToDisplay] = useState<
    Array<ScheduleType> | undefined
  >(undefined);
  ...
  useEffect(() => {
    if (user !== undefined && user.skateProfiles !== undefined) {
      const updatedSkateProfile = user.skateProfiles.find(
        skateProfile => skateProfile.id === currentSkateProfile?.id
      );
      if (updatedSkateProfile && updatedSkateProfile.schedules !== undefined) {
        updatedSkateProfile.schedules.forEach(schedule =>
          console.log(
            "id: " + schedule.id + "   zoneId: " + schedule.zones[0].id + "\n"
          )
        );
  ...
  const getAndSetSchedules = () => {
    if (
      ...
      if (
        ...
        currentSkateProfile.id !== undefined
      ) {
        Fetch.getSchedulesForSkateProfile(
  ...
  const updateASchedule = (scheduleIndex: number) => {
    if (schedulesToDisplay !== undefined) {
      ...
      navigation.navigate(
        "Schedule" as never,
        { updateMode: true, scheduleToUpdate: scheduleToUpdate } as never
      );
    }
  };
  ...
  return (
    ...
    >
      {user !== undefined && (
        <SkateProfiles
          profiles={user?.skateProfiles}
          value={currentSkateProfile}
          onValueChange={profile => {
            dispatch(setCurrentSkateProfile(profile));
          }}
        ></SkateProfiles>
      )}
      <AddScheduleElement
