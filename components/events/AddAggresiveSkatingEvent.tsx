...
const AddAggresiveSkatingEvent = ({ onPress }: Input) => {
  ...
  return (
    <PrimaryContainer
      styleInput={getStyle()}
      onPress={() => isAggresiveSkatingProfile() === true && onPress()}
    >
      <View
        style={{
      ...
      {isAggresiveSkatingProfile() === false && (
        <Text style={{ color: theme.colors.tertiary }} variant="labelSmall">
          Only available for aggresive skating
        </Text>
      )}
    </PrimaryContainer>
  );
