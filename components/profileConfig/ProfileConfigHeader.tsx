...
const ProfileConfigHeader = ({
  ...
  const {
    ...
    name,
    shortDescription,
  } = useSelector((state: any) => state.configProfile);
  const { userId, addingSkateProfile, user, JWTTokenResult } = useSelector(
      ...
      if (
        ...
        Fetch.postUser(
          ...
          (postedUser: User) => {
            ...
            dispatch(setCurrentSkateProfile(postedUser?.skateProfiles[0]));
          },
          () => console.log("Coudn't post user at profile creation")
        );
      } else {
        //TODO refresh token
      }
            ...
            Fetch.postSkateProfile(
              ...
              postedSkateProfile => {
                dispatch(
                  setUser({
                    ...user,
                    skateProfiles: [...user.skateProfiles, postedSkateProfile],
                  })
                );
              },
    ...
    return (
      <View style={{ justifyContent: "center", alignItems: "center" }}>
        <View style={{ flexDirection: "row" }}>
          ...
          <Text style={[styles.textCategory, { color: theme.colors.tertiary }]}>
            {skateType}
          </Text>
        </View>
        <View style={{ flexDirection: "row" }}>
        ...
        {gender != undefined && (
          <View style={{ flexDirection: "row" }}>
            ...
            <Text
              style={[styles.textCategory, { color: theme.colors.tertiary }]}
            >
              {gender}
            </Text>
          </View>
  ...
  return (
    <AppHeader>
      {backButton !== undefined && backButton === true && (
        <Appbar.BackAction
          style={{ left: scale(20), position: "absolute" }}
          onPress={goBack}
        />
      )}
      {backButton === false && (
        <Button
          disabled={false}
          text="LOGOUT"
          onPress={authenticationUtils.logOut}
          style={{ position: "absolute", left: scale(20) }}
        />
      )}
      ...
      {doneConfig === true && (
        <Button
          disabled={disabled}
          text="DONE"
          onPress={endProfileConfiguration}
          style={{ position: "absolute", right: scale(20) }}
        />
      )}
      <GeneralModal
