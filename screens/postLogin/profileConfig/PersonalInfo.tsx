...
import { ProfileConfigHeader, SelectGender, SelectNameAndDescription } from "../../../components/profileConfig";
import { setAge, setGender, setName, setShortDescription } from "../../../redux/configProfileState";

const PersonalInfo = () => {

    const {age, gender, name, shortDescription } = useSelector((state: any) => state.configProfile)
    ...
    useEffect(() => {
        if(selectedGender !== undefined && selectedAge !== undefined && choseName !== undefined 
            && choseName !== '' && description !== undefined && description !== '')
        ...
        return(
            <View style={[StyleSheet.absoluteFill, {justifyContent: 'center', alignItems: 'center', backgroundColor: theme.colors.primary}]}>
                    ...
                    <View style={{flexDirection: 'row'}}>
                        <SelectNumber descriptionText="Select Age" value={selectedAge} onChange={(val) => setSelectedAge(val)} range={{minimumValue: constants.minimumAge, maximumValue: constants.maximumAge}}></SelectNumber>
