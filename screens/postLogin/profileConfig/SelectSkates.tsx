...
const SelectSkates = () => {
  ...
  const { addingSkateProfile } = useSelector(
    (state: RootState) => state.appState
  );

  const [selectedSkateType, setSelectedSkateType] = useState<
    SkatesType | undefined
  >(skateType);
  ...
  const getCards = () => {
    ...
    const flipSelectedSkateType = (skateType: SkatesType) => {
      if (selectedSkateType != undefined && selectedSkateType === skateType) {
        ...
        setSelectedSkateType(skateType);
      }
    };

    return keys.map(skateType => {
      return (
        <SelectionCard
        ...
        >
          <Image
            style={styles.image}
            resizeMode="contain"
            source={{ uri: getImageUri(skateType) }}
          ></Image>
        </SelectionCard>
      );
    });
  ...
  const getBody = () => {
    return (
      <View style={[StyleSheet.absoluteFill, SpacingStyles.centeredContainer]}>
        <Text variant="headlineMedium" style={{ margin: verticalScale(15) }}>
          Select your skates:
        </Text>
        {getCards()}
      </View>
