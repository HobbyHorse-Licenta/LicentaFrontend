...
const CreateEvent = () => {
    ...
    if (fullScreenMap === false && mapRef.current !== null) {
      const coordinates = checkPointsArray.map(checkPoint => {
        return {
          ...
          longitude: checkPoint.location.long,
        };
      });
  ...
  const createEvent = () => {
    if (
      ...
      if (
        ...
        Fetch.postAggresiveSkatingEvent(
          JWTTokenResult.token,
          newAggresiveSkatingEvent,
          () => {
            console.log(
              "POSTED NEW AGGRESIVE EVEVNT SUCCESSFULLY"
  ...
  const canCreateEvent = (): boolean => {
    if (
      checkPointsArray === undefined ||
      checkPointsArray === null ||
      checkPointsArray.length < 2
    ) {
      return false;
    }
    if (
  ...
  const showAddMarkerButton = nativeEvent => {
    ...
    if (mapRef !== null && mapRef.current !== null) {
      mapRef.current.pointForCoordinate(coordinate).then(pointOnScreen =>
        setMarkerPopOverPosition({
          x: pointOnScreen.x,
          y: pointOnScreen.y + scale(70),
        })
      );
  ...
  const updateCheckpointCoordinates = (
    elementToUpdateIndex: number,
    changedCoordinates: LatLng
  ) => {
    setCheckPointsArray(
      checkPointsArray.map((checkpoint, index) => {
  ...
  const getMap = () => {
    return (
      ...
      >
        {mapsUtils.getCustomTrailMarkers(
          checkPointsArray,
          (index, changedCoordinated) =>
            updateCheckpointCoordinates(index, changedCoordinated),
          0
        )}
  ...
  const getPopOver = () => {
    return (
      <Popover
        ...
        from={
          new Rect(
            markerPopOverPosition.x + scale(15),
            markerPopOverPosition.y,
            0,
            0
          )
        }
      ...
      >
        <Button
          textColor="white"
          style={{ backgroundColor: "purple", borderRadius: 0 }}
          text="Add marker"
          onPress={() => {
            addCheckPoint();
            setSelectedLocation(undefined);
        ...
        {eventImageLoading !== true ? (
          <PrimaryContainer styleInput={{ width: width }}>
            ...
            <PrimaryContainer styleInput={{ width: width, height: height }}>
              <RectangularPicture
                onLoadingStatusChange={status => setEventImageLoading(status)}
                ratio={1.8}
                image={eventImage}
  ...
  const getMapContainer = () => {
    return (
      <PrimaryContainer styleInput={styles.mapContainer}>
        <Text style={{ flexWrap: "nowrap" }} variant="headlineSmall">
          Create a trail for your event
        </Text>
        <Text>
          You can make a trail by adding checkpoint on the map. A checkpoint is
          a place you want the trail to go through
        </Text>
        <Pressable onPress={() => setFullScreenMap(true)}>
          <View style={styles.map}>{getMap()}</View>
        </Pressable>
      </PrimaryContainer>
    );
  };
    ...
    return (
      <PrimaryContainer styleInput={{ padding: scale(10) }}>
        <View style={{ marginBottom: scale(25) }}>
          <Text variant="titleMedium" style={{ padding: scale(5) }}>
            You would go out with maximum:
          </Text>
          <View
            style={[SpacingStyles.centeredContainer, { flexDirection: "row" }]}
          >
            <SelectNumberOfPeople
              inputValue={
                numberOfPartners !== undefined ? numberOfPartners : null
              }
              onChange={nr => setNumberOfPartners(nr)}
            ></SelectNumberOfPeople>
            <Text style={{ marginLeft: scale(10) }}>skaters</Text>
          </View>
        </View>
        <Text variant="titleMedium" style={{ marginBottom: scale(7) }}>
          Event for skaters aged:
        </Text>
        <SelectAgeGap
          minimumAge={minimumAge !== undefined ? minimumAge : null}
          maximumAge={maximumAge !== undefined ? maximumAge : null}
          onMinimumAgeChange={age => setMinimumAge(age)}
  ...
  const getEverythingElse = () => {
    return (
      <View
        style={{ flex: 1, width: containersWidth, height: verticalScale(700) }}
      >
        <View style={[SpacingStyles.centeredContainer, { flex: 0.5 }]}>
          <SelectDays
            selectedDays={selectedDays}
            onSelectedDaysChange={selDays => setSelectedDays(selDays)}
          ></SelectDays>
        </View>
        <View style={[SpacingStyles.centeredContainer, { flex: 0.5 }]}>
          <SelectHourRange
  ...
  return (
    <Layout2Piece
      header={
        fullScreenMap === false ? (
          <GeneralHeader
            rightButtonEnable={canCreateEvent()}
            onBack={() => onBackFunction()}
            onRightButtonPress={() => createEvent()}
          ...
          <GeneralHeader
            ...
            onBack={() => onBackFunction()}
            onRightButtonPress={() => setFullScreenMap(false)}
