...
const MyEvents = () => {
  const { user, needsEventsRefresh, JWTTokenResult } = useSelector(
    ...
    if (currentSkateProfile !== undefined) {
      if (
        ...
        Fetch.getEventsForSkateProfile(
          ...
          joinedEvents => {
            setEvents(joinedEvents), setLoading(false);
          },
          () => {
            setEvents([]);
            uiUtils.showPopUp(
              "Error",
              "Database is not working\nWe couldn't load user events"
            );
            setLoading(false);
          }
        );
                  ...
                  <AggresiveEventCard
                    ...
                    onPress={() =>
                      navigation.navigate(
                        ...
                        { event: evnt, joined: true } as never
                      )
                    }
                  ></AggresiveEventCard>
                );
