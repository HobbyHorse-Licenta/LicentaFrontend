...
import { GenderDisplay } from "../../../components/eventDisplay";
...
const AggresiveEventDisplay = ({ route, navigation }) => {
  ...
  const getMap = () => {
    return (
      <MapView
        ref={mapRef}
        style={{ height: verticalScale(250), width: "100%", zIndex: 0 }}
        initialRegion={{
          ...
          longitudeDelta: mapsUtils.kMToLongitudes(
            2,
            customTrail.checkPoints[0].location.lat
          ),
        }}
      ...
      >
        ...
        {mapsUtils.drawRoute(
          customTrail.checkPoints,
          customTrail.checkPoints.length + 2
        )}
      </MapView>
    );
  };
  ...
  const skatersInfo = () => {
    return (
      <View style={SpacingStyles.centeredContainer}>
        ...
        >
          {suggestedSkateProfiles?.length === 0 &&
          attendingSkateProfiles?.length === 0 ? (
            <Text
              variant="headlineSmall"
              style={{ marginBottom: scale(2), textAlign: "center" }}
            >
              No skaters suggested or participating
            </Text>
          ) : (
