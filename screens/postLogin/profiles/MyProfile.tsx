...
const MyProfile = () => {
  ...
  const userInfo = () => {
    ...
    if (user !== undefined) {
      return (
        <PrimaryContainer
          styleInput={{ ...styles.profileContainer, ...styles.containerSize }}
        >
  ...
  const skateProfiles = () => {
    if (user !== undefined) {
      return (
        <TourGuideZone
          zone={1}
          text={`Hold skate profile to see its assigned skills ${SMILING_FACE_WITH_OPEN_MOUTH}`}
      ...
      header={
        <GeneralHeader
          ...
          menuItems={[
            {
              text: "Edit Profile",
              function: () => navigation.navigate("EditProfile" as never),
            },
