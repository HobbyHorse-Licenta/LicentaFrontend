...
const EditProfile = () => {
  ...
  const [editedImage, setEditedImage] = useState(
    user !== undefined ? user.profileImageUrl : undefined
  );
  ...
  const updateName = (typedName: string) => {
    if (validation.validateName(typedName) || typedName === "") {
      ...
      setEditedName(previousText => previousText);
    }
  };
        ...
        {user !== undefined ? (
          <View style={{ justifyContent: "center", alignItems: "center" }}>
            <PrimaryContainer styleInput={SpacingStyles.profilePicContainer}>
              <RoundPicture
                image={editedImage}
                onChange={imgUrl => setEditedImage(imgUrl)}
              ></RoundPicture>
            </PrimaryContainer>
            <TextInput
              style={[
            ...
            ></SelectNumber>
            <View
              style={{ width: scale(100), height: verticalScale(40) }}
            ></View>
          </View>
        ) : (
    ...
    <Layout2Piece
      header={
        <GeneralHeader
          ...
          onRightButtonPress={() => finishedProfileEdit()}
