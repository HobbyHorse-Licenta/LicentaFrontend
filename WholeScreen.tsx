...
export const NotifRefProvider =
  createContext<MutableRefObject<NotificationPopup | null> | null>(null);
  ...
  useEffect(() => {
    if (user !== undefined && user !== null) {
      if (
        ...
        user.skateProfiles !== null &&
        user.skateProfiles.length !== 0
      ) {
  ...
  const postExpoPushToken = (expoPushToken: string, connectedUser: User) => {
    if (expoPushToken !== undefined && expoPushToken !== null) {
      if (
        ...
        connectedUser !== null &&
        (connectedUser.pushNotificationToken === null ||
          connectedUser.pushNotificationToken === undefined ||
          connectedUser.pushNotificationToken.length === 0)
      ) {
        ...
        if (
          ...
          Fetch.putUser(
            ...
            () =>
              uiUtils.showPopUp(
                "Error",
                "Couldn't post notification token to database"
              )
          );
        } else {
          //TODO refresh token
        ...
        console.log(
          "User undefined at this moment; can't post notification token"
        );
      }
    }
  };
      ...
      user
        ...
        .then(idTokenResult => {
          ...
          if (validation.isJWTTokenExpired(idTokenResult)) {
            ...
            uiUtils.showPopUp(
              "TOKEN",
              "A EXPIRAT, ACUM TE SCOTEAM AFARA",
              () => nothing,
              6000
            );
            //dispatch(resetAppState());
            ...
            if (user.uid !== undefined) {
              ...
              if (
                ...
                !validation.isJWTTokenExpired(idTokenResult)
              ) {
                Fetch.getUser(
                  idTokenResult.token,
                  user.uid,
    ...
    return (
      <View>
        ...
        <NotifRefProvider.Provider value={popUp}>
          {(loading === true || internetConnected === undefined) && (
            <LoadingScreen></LoadingScreen>
          )}
          {loading === false && internetConnected === true && (
            <MainStack></MainStack>
          )}
          {loading === false && internetConnected === false && (
            <CheckInternetScreen></CheckInternetScreen>
          )}
        ...
        <View
          style={{
            ...
            height: 9,
            position: "absolute",
            top: Platform.OS === "ios" ? 0 : -verticalScale(30),
          }}
        >
          <NotificationPopup
            ref={popUp}
